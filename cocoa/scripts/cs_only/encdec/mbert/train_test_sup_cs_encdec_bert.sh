...
do
    echo "Train BERT only on CS data"
    PYTHONPATH=. python src/main.py --schema-path "cross-val/schema_cs.json" --scenarios-path "data/scenarios.json" \
    --train-examples-paths "cross-val/train_"$i".json"  --word-embed-size 768 --word-embed-type "BERT"  \
    ...
    --optimizer adagrad --print-every 50 --model $nn_model --gpu 1 --rnn-size 100 --grad-clip 0 --num-items 12 \
