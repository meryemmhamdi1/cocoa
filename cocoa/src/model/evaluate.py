...
def pred_to_token(preds, stop_symbol, remove_symbols, textint_map, num_sents=None):
    ...
    for pred, n in zip(preds, num_sents):
...
class Evaluator(object):
    def __init__(self, data, model, splits=('dev',), batch_size=1, verbose=True):
        ...
        self.num_batches = {split: data.__next__() for split, data in self.eval_data.items()}

        # For post-processing of generated utterances
        self.stop_symbol = self.vocab.to_ind_word(markers.EOS)
        #self.remove_symbols = map(self.vocab.to_ind_word, (markers.EOS, markers.PAD))
        self.remove_symbols = map(self.vocab.to_ind_word, (markers.PAD,))

    def dataset(self):
        '''
        Iterator over all datasets.
        '''
        for split, generator in self.eval_data.items():
            yield split, generator, self.num_batches[split]

    def get_res(self, size, dialogue_turn):
        idx_list = [idx + 1 for idx, val in enumerate(dialogue_turn) if val == "</s>"]

        res = [dialogue_turn[ii: jj] for ii, jj in zip([0] + idx_list, idx_list + ([size] if idx_list[-1] != size else []))]
        return res
    ...
    def test_bleu(self, sess, test_data, num_batches):
        ...
        for i in tqdm(range(num_batches)):
            ...
            for batch in dialogue_batch['batch_seq']:
                ...
                if self.copy:
                    preds = graphs.copy_preds(preds, self.vocab.size_word)
                ...
                print(batch['decoder_tokens'])
                for dialogue_turn in batch['decoder_tokens']:
                    size = len(dialogue_turn)
                    if size > 0:
                        res = self.get_res(size, dialogue_turn)
                        for utterance in res:
                            utt_length.append(len(utterance))
                    else:
                        utt_length.append(0)

                # Success rate (overall)
                for i, dialogue_turn in enumerate(batch['decoder_tokens']):
                    ## dialogue turn
                    size = len(dialogue_turn)
                    if size > 0 and len(dialogue_turn) == len(pred_tokens[i]):
                        res = self.get_res(size, dialogue_turn)

                        ref_exit_utterance = res[-1]
                        res_ref = res

                        ## preds
                        size = len(pred_tokens[i])
                        res = self.get_res(size, pred_tokens[i])

                        pred_exit_utterance = res[-1][1:]

                        for index_cs in range(0, len(res)):

                            print("BEFORE REMOVING CS TAGS ==> ref_exit_utterance:", res_ref[index_cs], "pred_exit_utterance:", res[index_cs])

                        if "<select>" in ref_exit_utterance:
                            ref_exit_utterance_mod = ["<select>"] + [ref_exit_utterance[2]] + ["</s>"]

                            print("dialogue_turn:", dialogue_turn, " ref_exit_utterance:", ref_exit_utterance_mod,
                                  " pred_exit_utterance:", pred_exit_utterance)

                            if compare(ref_exit_utterance_mod, pred_exit_utterance):
                                success_rate.append(1)
                            else:
                                success_rate.append(0)

                print("SO FAR SUCCESS RATE:", np.mean(success_rate))
        ...
        return bleu, entity_f1, np.mean(utt_length), np.mean(success_rate)
    ...
    def _print_batch(self, batch, preds, targets, bleu_scores, graphs, attn_scores, probs):
        ...
        print ('-------------- batch ----------------')
        for i, (target, pred, bleu) in enumerate(zip_longest(targets, preds, bleu_scores)):
            # Skip padded turns
            ...
            if probs is not None:
                ...
                for j, w in enumerate(pred):
                    ...
                    for id_ in topk:
                        prob = probs[j][i][id_]
                        if id_ < self.vocab.size_word:
                            print self.vocab.to_word(id_), prob
                        else:
                            print graphs.graphs[i].nodes.to_word(id_ - self.vocab.size_word), prob
            """
    ...
    def update_bleu_stats(self, stats, batch_preds, batch_targets):
        for preds, targets in zip(batch_preds, batch_targets):
            if len(targets) > 0:
                stats = [sum(scores) for scores in zip(stats, get_bleu_stats(preds, targets))]
        return stats
