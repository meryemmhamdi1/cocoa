...
def add_graph_arguments(parser):
    ...
    parser.add_argument('-GraphMetadata-max-degree', type=int, default=10, help='Maximum degree of a node in the graph')
...
class GraphMetadata(object):
    ...
    def __init__(self, schema, entity_map, relation_map, utterance_size, max_num_entities, max_degree=10, entity_hist_len=2, max_num_items=10):
        ...
        # Item names/ids
        node_types.add_words([item_to_str(i) for i in range(max_num_items)])
        #node_types.add_words(['item', 'attr'])
        self.feat_inds = {'degree': (0, degree_size),
                          'node_type': (degree_size, node_types.size_word),
                          'rel_degree': (degree_size + node_types.size_word, rel_degree_size),
                         }
...
class GraphBatch(object):
    ...
    def _max_num_nodes(self):
        return max([graph.nodes.size_word for graph in self.graphs])
    ...
    def update_entities(self, tokens, stage=None):
        ...
        for graph, toks in zip(self.graphs, tokens):
    ...
    def copy_targets(self, targets, vocab_size):
        ...
        We assume that entities in targets are mapped by entity_map and offset by vocab.size_word.
        '''
        ...
        for i, graph in enumerate(self.graphs):
            for j, t in enumerate(new_targets[i]):
                if t >= vocab_size:
                    ...
                    new_targets[i][j] = graph.nodes.to_ind_word(entity) + vocab_size
        return new_targets
            ...
            for j, t in enumerate(new_preds[i]):
                if t >= vocab_size:
                    try:
                        ...
                        new_preds[i][j] = 0  # <unk>
                        continue
                    new_preds[i][j] = Graph.metadata.entity_map.to_ind_word(entity) + vocab_size
        return new_preds
    ...
    def _entity_to_node_id(self, entities):
        ...
        for entity_id, node_id, graph in zip(np.nditer(entities), np.nditer(node_ids, op_flags=['readwrite']), graph_iter):
            entity_id = entity_id[()]
            if entity_id != -1:
                try:
                    # [()]: entity_id is a 0-dim ndarray
                    node_id[...] = graph.nodes.to_ind_word(Graph.metadata.entity_map.to_word(entity_id))
                except KeyError:
...
class Graph(object):
    ...
    def reset(self):
        ...
        self.node_ids = np.arange(self.nodes.size_word, dtype=np.int32)
        self.entity_ids = np.array([Graph.metadata.entity_map.to_ind_word(self.nodes.to_word(i)) for i in range(self.nodes.size_word)], dtype=np.int32)
    ...
    def _add_path(self, node1, relation, node2):
        node1_id = self.nodes.to_ind_word(node1)
        node2_id = self.nodes.to_ind_word(node2)
        rel = Graph.metadata.relation_map.to_ind_word(relation)
        irel = Graph.metadata.relation_map.to_ind_word(inv_rel(relation))
    ...
    def load_kb(self, kb):
        ...
        for attr_node, ent_set in attr_ents.items():
    ...
    def read_utterance(self, tokens, stage=None):
        ...
        node_ids = [self.nodes.to_ind_word(x[1]) for x in tokens if is_entity(x)]
    ...
    def _update_entity_ids(self, entities):
        self.entity_ids = np.concatenate([self.entity_ids,
                   [Graph.metadata.entity_map.to_ind_word(entity) for entity in entities]], axis=0)
    ...
    def get_features(self):
        nodes = [self.nodes.to_word(i) for i in range(self.nodes.size_word)]
    ...
    def get_feat_vec(self, raw_feats):
        ...
        for i, (degree, node_type) in enumerate(raw_feats):
            ...
            f[i][self._get_index('node_type', Graph.metadata.node_types.to_ind_word(node_type))] = 1
