...
from src.model.word_embedder import WordEmbedder, MUSEWordEmbedder
...
def build_model(schema, mappings, args):
    ...
    if args.model == 'encdec':
        ...
        if args.model == 'attn-encdec':
            ...
            decoder = CopyGraphDecoder(args.rnn_size, vocab._word, graph_embedder, rnn_type=args.rnn_type, num_layers=args.num_layers, bow_utterance=args.bow_utterance, checklist=(not args.no_checklist), dropout=args.dropout, sample_t=sample_t, sample_select=sample_select, update_graph=update_graph, node_embed_in_rnn_inputs=node_embed_in_rnn_inputs)
        model = GraphEncoderDecoder(encoder_word_embedder, decoder_word_embedder, graph_embedder, encoder, decoder, pad, select)
    else:
        raise ValueError('Unknown model')
    return model
...
class BasicEncoder(object):
    ...
    def _build_inputs(self, input_dict):
        with tf.name_scope(type(self).__name__+'/inputs'):
            ...
            self.last_inds = tf.compat.v1.placeholder(tf.int32, shape=[None], name='last_inds')

            ## BERT Features
            self.batch_input_ids = tf.compat.v1.placeholder(tf.int32, shape=[None, None], name="input_ids") # shape [batch_size, seq_len]
            self.batch_mask = tf.compat.v1.placeholder(tf.int32, shape=[None, None], name="batch_mask") # shape [batch_size, seq_len]
            self.batch_segment_ids = tf.compat.v1.placeholder(tf.int32, shape=[None, None], name="batch_segment_ids") # shape [batch_size, seq_len]
            self.batch_label_words = tf.compat.v1.placeholder(tf.int32, shape=[None, None], name="batch_label_words")
    ...
    def build_model(self, word_embedder, input_dict, time_major=True, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            ...
            model = modeling.BertModel(
                config = bert_config,
                is_training=True, # make static for now then we will see what happens
                input_ids=self.batch_input_ids,
                input_mask=self.batch_mask,
                token_type_ids=self.batch_segment_ids,
                use_one_hot_embeddings=False # asks if training on TPU (True) vs GPU/CPU (False
            )

            bert_output_layer = model.get_sequence_output() # [batch_size, seq_len, hidden_size]
            bert_output_sent = model.get_pooled_output() # [batch_size, hidden_size]
            #print("bert_output_sent:", bert_output_sent.shape)
            #tf.where(tf.equal(self.batch_label_words))
            ...
            rnn_outputs, states = tf.compat.v1.nn.dynamic_rnn(cell, inputs, dtype=tf.float32)
            self._build_output_dict(rnn_outputs, states, bert_output_sent)

    def _build_output_dict(self, rnn_outputs, rnn_states, bert_output_sent):
        final_state = bert_output_sent #tf.concat(self._get_final_state(rnn_states)
    ...
    def get_feed_dict(self, **kwargs):
        ...
        feed_dict[self.batch_input_ids] = kwargs.pop('batch_input_ids')
        feed_dict[self.batch_mask] = kwargs.pop('batch_mask')
        feed_dict[self.batch_segment_ids] = kwargs.pop('batch_segment_ids')
...
class GraphEncoder(BasicEncoder):
    ...
    def _build_graph_variables(self, input_dict):
        if 'utterances' in input_dict:
            ...
            self.utterances = (tf.compat.v1.placeholder(tf.float32, shape=[None, None, self.graph_embedder.config.utterance_size], name='encoder_utterances'),
                               tf.compat.v1.placeholder(tf.float32, shape=[None, None, self.graph_embedder.config.utterance_size], name='decoder_utterances'))
    ...
    def _build_inputs(self, input_dict):
        ...
        with tf.name_scope(type(self).__name__+'/inputs'):
            ...
            self.update_entities = tf.compat.v1.placeholder(tf.int32, shape=[None, None], name='update_entities')
            # Entities in the current utterance. Non-entity words are -1.
            self.entities = tf.compat.v1.placeholder(tf.int32, shape=[None, None], name='entities')
...
class BasicDecoder(BasicEncoder):
    def __init__(self, rnn_size, num_symbols, rnn_type='lstm', num_layers=1, dropout=0, sample_t=0, sample_select=None):
    ...
    def _build_inputs(self, input_dict):
        ...
        with tf.name_scope(type(self).__name__+'/inputs'):
            self.matched_items = tf.compat.v1.placeholder(tf.int32, shape=[None], name='matched_items')
    ...
    def decode(self, sess, max_len, batch_size=1, stop_symbol=None, **kwargs):
        ...
        batch_input_ids = kwargs.pop('batch_input_ids')
        batch_mask = kwargs.pop('batch_mask')
        batch_segment_ids = kwargs.pop('batch_segment_ids')
...
class GraphDecoder(GraphEncoder):
    ...
    def _build_inputs(self, input_dict):
        ...
        with tf.name_scope(type(self).__name__+'/inputs'):
            self.matched_items = tf.compat.v1.placeholder(tf.int32, shape=[None], name='matched_items')
            self.init_checklists = tf.compat.v1.placeholder(tf.int32, shape=[None, None, None], name='init_checklists')
...
class GraphEncoderDecoder(BasicEncoderDecoder):
    def __init__(self, encoder_word_embedder, decoder_word_embedder, graph_embedder, encoder, decoder, pad, select, scope=None):
        self.graph_embedder = graph_embedder
