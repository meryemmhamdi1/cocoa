...
recurrent_cell = {'rnn': tf.compat.v1.nn.rnn_cell.BasicRNNCell, #tf.contrib.rnn.core_rnn_cell.BasicRNNCell,
...
def build_rnn_cell(rnn_type, rnn_size, num_layers, keep_prob):
    ...
    '''
    if rnn_type == 'lstm':
        cell = recurrent_cell[rnn_type](rnn_size, state_is_tuple=False)
    else:
        cell = recurrent_cell[rnn_type](rnn_size)
    if num_layers > 1:
        cell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell, input_keep_prob=keep_prob)
...
class AttnRNNCell(object):
    ...
    def _output_concat(self, output, attn):
        return tf.concat([output, attn], axis=1)

    def _output_project(self, output, attn, project_size):
        with tf.variable_scope("AttnOutputProjection"):
            output_new = tf.contrib.layers.fully_connected(tf.concat([output, attn], axis=1), project_size)#, False)
            print(output_new)
            new_output = tf.tanh(output_new)
        return new_output
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            # RNN step
            new_inputs = tf.concat([inputs, prev_attn], axis=1)
