...
class TestPreprocess(object):
    ...
    @pytest.fixture(scope='session')
    def generator(self, args, examples, lexicon, schema):
        return DataGenerator(args, examples, examples, None, schema, lexicon, 5, use_kb=True)
    ...
    def test_preprocess(self, generator, examples, capsys):
        ...
        with capsys.disabled():
            ...
            for i, (agent, turn) in enumerate(zip(dialogue.agents, dialogue.turns[0])):
                print ('agent=%d' % agent)
    ...
    def test_create_batches(self, generator, test_dialogue_batch, capsys):
        ...
        with capsys.disabled():
            for i in range(2):  # Which perspective
                batch = batches[i]
                print ('\n========== Example batch ==========')
                for j in range(1):  # Which batch
    ...
    def test_generator(self, schema, generator):
        ...
        num_batches = generator.__next__()
