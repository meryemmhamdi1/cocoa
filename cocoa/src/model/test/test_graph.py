...
class TestGraph(object):
    def test_mappings(self, graph, capsys):
        with capsys.disabled():
            print('Entity map:')
            graph.metadata.entity_map.dump_word()
            print ('Relation map:')
            graph.metadata.relation_map.dump_word()

    def test_nodes(self, graph, capsys):
        with capsys.disabled():
            print ('%d nodes in graph:' % graph.nodes.size)
            graph.nodes.dump_word()
    ...
    def test_add_entity(self, graph, capsys):
        ...
        assert graph.nodes.to_ind_word(('facebook', 'company')) == 10
    ...
    def test_read_utterance(self, graph, capsys):
        ...
        alice = graph.nodes.to_ind_word(('alice', 'person'))
        google = graph.nodes.to_ind_word(('google', 'company'))
    ...
    def test_copy(self, graph_batch, metadata, preprocessor, vocab, textint_map):
        alice = metadata.entity_map.to_ind_word(('alice', 'person')) + vocab.size_word
        hiking = metadata.entity_map.to_ind_word(('hiking', 'hobby')) + vocab.size_word
        targets = np.array([[0, alice, hiking],
                            [1, alice, hiking]])
        new_targets = graph_batch.copy_targets(targets, vocab.size_word)
        assert graph_batch.graphs[0].nodes.to_word(new_targets[0][1]-vocab.size_word) == ('alice', 'person')
        assert graph_batch.graphs[0].nodes.to_word(new_targets[0][2]-vocab.size_word) == ('hiking', 'hobby')
        preds = graph_batch.copy_preds(new_targets, vocab.size_word)
    ...
    def test_checklist(self, graph_batch, vocab, metadata):
        ...
        alice_node_id = graph_batch.graphs[0].nodes.to_ind_word(('alice', 'person'))
        hiking_node_id = graph_batch.graphs[1].nodes.to_ind_word(('hiking', 'hobby'))
