...
class GraphEmbedder(object):
            ...
            with tf.variable_scope('NodeEmbedding'):
                with tf.variable_scope('InitNodeEmbedding') as scope:
                    ...
                    if self.config.use_entity_embedding:
                        ...
                        print("batch_embedding_lookup(utterances[0], node_ids).shape:", batch_embedding_lookup(utterances[0], node_ids).shape)
                        print("batch_embedding_lookup(utterances[1], node_ids).shape:", batch_embedding_lookup(utterances[1], node_ids).shape)
                ...
                if self.config.mp_iters > 0:
                    ...
                    for i in range(self.config.mp_iters-1):
                        print("i:", i)
                        if i > 0:
                            tf.get_variable_scope().reuse_variables()
                        node_embeds.append(mp(node_embeds[-1]))

        context = tf.concat(node_embeds, axis=2)
    ...
    def _update_utterance(self, entity_indices, utterance, curr_utterances):
        ...
        inds = tf.concat([batch_inds, node_inds, utterance_inds], axis=1)
        ...
        if self.config.learned_decay:
            with tf.variable_scope('UpdateUtterance', reuse=self.update_initialized):
                weight = tf.sigmoid(batch_linear(tf.concat([curr_utterances, new_utterance], axis=2), 1, True))  # (batch_size, num_nodes, 1)
