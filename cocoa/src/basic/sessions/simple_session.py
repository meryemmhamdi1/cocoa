...
from src.model.preprocess import tokenize_type_based as tokenize
...
class SimpleSession(Session):
    ...
    def count_entity(self):
        ...
        for item in self.kb.items:
            for attr_name, entity in item.items():
    ...
    def weight_entity(self):
        ...
        # Scale counts to [0, 1]
        entity_weights = {entity: count / N for entity, count in entity_counts.items()}
    ...
    def fact_to_str(self, fact, num_items, include_count=True, prefix=False, question=False):
        ...
        for entities, count in fact:
            ...
            if prefix:
                new_str = []
                for entity, s in zip(entities, entities_str):
