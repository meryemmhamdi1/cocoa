...
def filter(raw_evals, uuid_to_chats):
    ...
    print ('Good scenarios:', sum([1 if len(a) == 4 else 0 for s, a in scenario_to_agents.items()]))
    for scenario_id, agents in scenario_to_agents.items():
        if len(agents) == 4:
...
def analyze(question_scores, uuid_to_chat, preprocessor):
    ...
    # plot
    corr = []
    for stat_name, question_scores in examples.items():
...
def visualize(viewer_mode, html_output, question_scores, uuid_to_chat):
    ...
    for dialogue_id, agent_responses in dialogue_responses.items():
        ...
        scenario_to_chats[scenario_id].add(dialogue_id)
    #for s, c in scenario_to_chats.items():
    #    assert len(c) >= 4
...
def summarize(question_scores):
    ...
    for summary_stat in ('mean',):
        ...
        for question, agent_scores in question_scores.items():
            if question == 'comments' or question.endswith('text'):
                continue
            results = [(agent, summarize_scores(scores, summary_stat), get_total(scores)) for agent, scores in agent_scores.items()]
