...
def count_to_entropy(counts):
    ...
    probs = np.array([v / total for k, v in counts.items()])
...
def get_attr_prop(attr_name, entity_name, kb):
    ...
    attr_uniq_value_counts = {k: len(v) for k, v in attr_values.items()}
...
def analyze_strategy(all_chats, scenario_db, preprocessor, text_output, lm):
    ...
    unigram_counts = {k[0]: v for k, v in ngram_counts[1].items()}
    ...
    return {'speech_act': {k: speech_act_summary_map[k] / total for k in speech_act_summary_map.keys()},
            ...
            'entity_mention': {k: np.mean(v) for k, v in entity_mention_summary_map['first'].items()},
...
def get_average_time_taken(all_chats, scenario_db, alphas=None, num_items=None):
    ...
    print("scenario_db:", scenario_db.scenarios_map.keys())
    #print("all_chats:", [chat["scenario_uuid"] for chat in all_chats])
    for chat in all_chats:
        print("------uuid:", chat["scenario_uuid"])
...
def get_turns_vs_completed(all_chats):
    ...
    return {k: v['complete']['sum'] for k, v in num_turns_dict.items()}
    ...
    for chat in all_chats:
        if chat["outcome"] is not None:
            ...
            logstats.update_summary_map(num_select_dict[num_select],
                                        {'complete': 1 if chat["outcome"]["reward"] == 1 else 0})
    return {k: v['complete']['sum'] for k, v in num_select_dict.items()}
...
def get_unigram_utterance(n, counts):
    start = ((START,), ())
    unigram_counts = [(k, sum(v.values())) for k, v in counts.items() if k != start]
    return get_topk_utterance(n, unigram_counts)


def get_bigram_utterance(n, counts):
    bigram_counts = [((k1, k2), v) for k1, d in counts.items() for k2, v in d.items()]
...
def print_strategy_stats(stats):
    ...
    for k, v in dialogue_stats.items():
        print ('%s: %.3f' % (k, v))
    ...
    for n in range(1, 4):
        sorted_words = sorted(ngram_counts[n].items(), key=lambda x: x[1], reverse=True)
