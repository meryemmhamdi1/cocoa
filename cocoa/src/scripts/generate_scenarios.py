...
import numpy as np
import sys
sys.path.append("/Users/d22admin/USCGDrive/Spring2020/CS644NLDialog/Project/cocoa/src/basic/")
...
def generate_scenario(schema):
    ...
    if args.random_items:
        num_items = np.random.choice(range(args.min_items, args.max_items+1))
    ...
    if random_attributes:
        ...
        num_attributes = min(np.random.choice(range(args.min_attributes, args.max_attributes)), len(schema.attributes))
    ...
    print("schema:", schema)
    for attr, alpha in alphas.items():
        n = min(len(schema.values[attr.value_type]), num_values)
        values[attr.name] = random.sample(schema.values[attr.value_type], n)
        print("attr:", attr.name, " attr.value_type:", schema.values[attr.value_type], " n:", len(schema.values[attr.value_type]), values[attr.name])
        for agent in agents:
    ...
    for i in range(100000):
        ...
        for agent in agents:
            ...
            for attr in scenario_attributes:
                ...
                #print("attr:", attr.name, "distrib:", distrib)
